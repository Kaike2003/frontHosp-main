...
function ScrollSuite() {
    ...
    const callBack = useCallback(async () => {
        ...
        await api.get(`/publico/listarTodosQuartoSuite/20`).then((res) => {
            setData(res.data)
        }).catch((error) => {
            console.log(error)
        })
    ...
    const fetchMoreData = async () => {
        alert("funcionando")
        await api.get(`/publico/listarTodosQuartoSuite/${limite}`).then((res) => {
            setData(res.data)
            setLimite(limite + 10)
        }).catch((error) => {
            console.log(error)
