...
function TabelaPendente() {
    ...
    const callBackReserva = useCallback(() => {
        ...
        api.get(`/funcionalidade/listarReservasPendentes/${storageEmail}`).then(async (sucesso) => {
            ...
            <Box>
                ...
                <IconButton
                    ...
                    onClick={() => {
                        ...
                        localStorage.setItem("@Auth:idReserva", row.original.id)
                        localStorage.setItem("@Auth:idQuarto", row.original.quartoId)

                        setReservaId(row.original.id, row.original.quartoId)
                        navigate("/hospedaria/cliente/reserva/editar")
